...
async function getFeatureCost(feature) {
  try {
    const [[row]] = await query('SELECT cost FROM token_costs WHERE feature_key = $1', [feature]);
...
async function callOpenRouter(prompt, model = DEF_MODEL, maxTokens = 1500) {
  if (!API_KEY) throw new Error('Chưa cấu hình OPENROUTER_API_KEY');
...
router.post('/chat', authenticate, async (req, res) => {
  try {
    const { feature = 'hint', prompt = '', model = DEF_MODEL, maxTokens = 1500 } = req.body;
    ...
    if (cost > 0) {
      ...
      if (!ok) {
        const [[u]] = await query('SELECT tokens FROM users WHERE id = $1', [userId]);
    ...
    const [[{ tokens: tokensLeft }]] = await query('SELECT tokens FROM users WHERE id = $1', [userId]);
...
router.post('/generate-questions', authenticate, async (req, res) => {
  try {
    const { subject = 'Toán học THPT', topic = 'Tổng hợp', count = 5, difficulty = 'trung bình', model = DEF_MODEL } = req.body;
    const userId = req.user.id;
    const role = req.user.role;
    ...
    const prompt = `Bạn là giáo sư chuyên gia tạo câu hỏi trắc nghiệm.
Môn: ${subject} | Chủ đề: ${topic} | Độ khó: ${diffMap[difficulty] || 'TRUNG BÌNH'}

Trả về ĐÚNG ${count} câu dưới dạng JSON array:
[{"text":"...","opts":["...","...","...","..."],"correct":0,"explanation":"..."}]`;
    ...
    try {
      ...
      return res.status(422).json({ error: 'AI trả về JSON không hợp lệ.' });
    }

    const [[{ tokens: tokensLeft }]] = await query('SELECT tokens FROM users WHERE id = $1', [userId]);
    return res.json({ questions, cached: false, model: result.model, tokensUsed: cost, tokensLeft: role === 'admin' ? null : tokensLeft });
  } catch (err) {
